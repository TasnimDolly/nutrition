    ...
    return (
      <>
        <TableContainer sx={{ textAlign: "center" }}>
          <Table
            sx={{ background: "#FFFFFF", maxWidth: 850, direction: "rtl" }}
          >
            <TableHead>
            ...
            <TableBody>
              {rows.map((row) => (
                <>
                  <TableRow
                    key={row.id}
                    sx={{ height: "auto", display: "table-row" }}
                  >
                    <TableCell
                      sx={{
                        maxWidth: "50px",
    ...
    <>
      ...
      >
        <Stack direction="row" justifyContent="space-between">
          <DialogTitle>
            <Typography variant="h4"> عرض المشتركين</Typography>
          </DialogTitle>
          <Box>
            <Button
              onClick={() => setActive("timeline")}
              variant={active == "timeline" ? "contained" : "standard"}
            >
              خط زمني
            </Button>{" "}
            <Button
